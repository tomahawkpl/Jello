...
void JNICALL init(JNIEnv *env, jclass dis, jobject pagedFileArg, jobject listPageArg, jshort pageSizeArg, 
		...
		jint histogramClassesArg, jint cacheSizeArg) {
	int i;
	initIDs(env);
...
int getBestId(short freeSpace) {
	struct CacheElement *element = cache;
	struct CacheElement *best = NULL;
	int bestExtraSpace = 0;
...
void updateCache(int id, short freeSpace) {
	int leastSpace = -1;
	...
	while (element != NULL) {
		if (cacheCount == cacheLimit && (leastSpace == -1 || element->freeSpace < leastSpace)) {
...
int acquirePageAO(JNIEnv *env) {
	int id;
	id = getBestId(pageSize);
	if (id != CacheNoPage) {
...
int acquireRecordAO(JNIEnv *env, jobject record, jint length) {
	...
	for (i = 0; i < chunks; i++) {
		id = getBestId(pageSize - spaceToSpare);

		if (id != CacheNoPage) {
			__android_log_print(ANDROID_LOG_INFO, "Jello",  "page from cache: %d",id);
			freeSpace = getFreeSpace(id);
			reservedOnThisPage = (leftToAcquire < freeSpace) ? (short) leftToAcquire : freeSpace;

			r = reserveBlocks(env, record, id, reservedOnThisPage);

			updateCache(id, freeSpace - r);
			...
			id = (*env)->CallIntMethod(env, pagedFile, midPagedFileAddPages, 1);
			__android_log_print(ANDROID_LOG_INFO, "Jello",  "new page: %d",id);
			...
			r = reserveBlocks(env, record, id, reservedOnThisPage);
			__android_log_print(ANDROID_LOG_INFO, "Jello",  "3");


			updateCache(id, freeSpace - r);
			updateHistogram(id, freeSpace, freeSpace - r);
			__android_log_print(ANDROID_LOG_INFO, "Jello",  "4");
	...
	__android_log_print(ANDROID_LOG_INFO, "Jello",  "5");
	freeSpaceInfoCommit(env);
	__android_log_print(ANDROID_LOG_INFO, "Jello",  "6");
	return 1;
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
	...
	nm[0].name = "init";
	nm[0].signature = "(Lcom/atteo/jello/store/PagedFile;Lcom/atteo/jello/store/ListPage;SSISSSIII)V";
	nm[0].fnPtr = init;
